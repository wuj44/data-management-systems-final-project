...
ORDER BY ROUND(AVG(Reviewer_Score), 0) DESC, COUNT(*) DESC;
...
FROM reviews
  ...
  JOIN hotel_info USING (Hotel_Address)
GROUP BY Hotel_Name, Hotel_City
ORDER BY COUNT(*) DESC, ROUND(AVG(Reviewer_Score), 0) DESC;
...
FROM reviews
  ...
  JOIN hotel_info USING (Hotel_Address)
GROUP BY Hotel_Name, Hotel_City
ORDER BY Businesses_1km DESC, ROUND(AVG(Reviewer_Score), 0) DESC;
...
-- create a view to show hotel overview searched by hotel name
DROP VIEW IF EXISTS hotels;
CREATE VIEW hotels AS
SELECT Hotel_Name, Hotel_City, Hotel_Address, ROUND(AVG(Reviewer_Score), 0) AS Average_Score, 
	   COUNT(*) AS Total_Number_of_Reviews, Room_Type, Businesses_100m, 
       Businesses_1km, Businesses_5km
FROM reviews
JOIN hotel_overview USING (Hotel_ID)
JOIN hotel_info USING (Hotel_Address)
GROUP BY Hotel_Name, Hotel_City;